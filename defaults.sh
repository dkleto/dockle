...
if ! declare -p $contargs | grep -iq "Declare -A contargs="; then
    declare -A contargs
    contargs=(
        [$dbimage]="$commontime $dbdata"
        [$webimage]="$commontime --link $dbcont:$dbcont $webdata -v `pwd`/www:/var/www/$sitedir -v `pwd`/logs:/var/log/sitelogs/$sitedir"
    );
fi
